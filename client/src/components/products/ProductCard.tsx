...
const ProductCard: React.FC<ProductCardProps> = ({ productName, productImage, description, uploadedBy, price, quantity, handleAddtoCart }) => {
    ...
    return (
        <>
            <div className="w-full md:max-w-[296px] min-[1400px]:max-w-[320px] flex flex-col shadow-lg justify-between bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700">
                <img className=" rounded-t-lg mb-4 max-h-[190px] max-w-[190px] m-auto py-2" src={productImage} alt="product image" />
                <div className="px-5 pb-5">
                    <div>
                        <h3 className="text-xl font-bold text-gray-900 dark:text-white">{productName.length > 25 ? `${productName.slice(0, 20)}...` : productName}</h3>
                        <h5 className="text-sm tracking-tighter mb-4 text-gray-700 leading-5 dark:text-white">{description.length > 40 ? `${description.slice(0, 60)}...` : description}</h5>
                        <h5 className="text-xs tracking-tighter capitalize mb-4 text-gray-900 leading-5 dark:text-white">upload by : <span className="font-semibold">{uploadedBy}</span></h5>
